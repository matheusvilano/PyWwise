...
class GUID(str):
    ...
    def __new__(cls, guid: str):
        ...
        if guid[0] != '{' or guid[-1] != '}':
            raise ValueError("GUID string format is incorrect. The string must be enclosed in curly brackets ('{' and '}').")
        if guid[9] != '-' or guid[14] != '-' or guid[19] != '-' or guid[24] != '-':
            raise ValueError("GUID string format is incorrect. Indexes 9, 14, 19, and 24 should all contain a dash (`-`).")
